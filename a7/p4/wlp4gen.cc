...
bool isControl(std::shared_ptr<Node> n) {
    std::string ifRule = "IF LPAREN test RPAREN LBRACE statements RBRACE ELSE LBRACE statements RBRACE";
    std::string whileRule = "WHILE LPAREN test RPAREN LBRACE statements RBRACE";
    return n->getTo() == ifRule || n->getTo() == whileRule;
}   

void codeBinaryOp(std::shared_ptr<Node> n, SymbolTable &varTable); // decalred so code can use it
void codeComparison(std::shared_ptr<Node> n, SymbolTable &varTable); // decalred so code can use it
void codeControl(std::shared_ptr<Node> n, SymbolTable &varTable); // decalred so code can use it
...
void doDcls(std::shared_ptr<Node> dcls, SymbolTable &varTable) {
    ...
    if (dcls->rule != "dcls dcls dcl BECOMES NUM SEMI") { // validation
        return;
    }
