...
bool isAlloc(std::shared_ptr<Node> n) {
    return n->getTo() == "NEW INT LBRACK expr RBRACK";
}

bool isDelete(std::shared_ptr<Node> n) {
    return n->getTo() == "DELETE LBRACK RBRACK expr SEMI";
}
...
void codeAlloc(std::shared_ptr<Node> factor, SymbolTable &varTable); // declared so code can use it
void codeDelete(std::shared_ptr<Node> statement, SymbolTable &varTable); // declared so code can use it
...
void code(std::shared_ptr<Node> expr, SymbolTable &varTable) {
    ...
    } else if (isAlloc(expr)) {
        codeAlloc(expr, varTable);
    } else {
...
void codeStatement(std::shared_ptr<Node> statement, SymbolTable &varTable) {
    ...
    if (to == "lvalue BECOMES expr SEMI") { // lvalue statement
        ...
        if (lvalueNode->rule == "lvalue ID") { // case: lvalue -> ID
            ...
            code(factorNode, varTable); // FIX HERE
            pop(5);
...
void codeComparison(std::shared_ptr<Node> n, SymbolTable &varTable) {
    ...
    pop(5);
    std::string out;
    std::string slt = leftNode->type == ptrType && rightNode->type == ptrType ? "sltu" : "slt";
    if (op == EQ) {
        out = slt + " $6, $3, $5\n"+ slt + " $7, $5, $3\nadd $3, $6, $7\nsub $3, $11, $3";
    } else if (op == NE) {
        out = slt + " $6, $3, $5\n"+ slt + " $7, $5, $3\nadd $3, $6, $7";
...
void codeAlloc(std::shared_ptr<Node> factor, SymbolTable &varTable) {
    auto exprNode = factor->children.at(3);
    code(exprNode, varTable);
    std::cout << "add $1, $3, $0 ; $1: number of words me mem being req" << std::endl;
    push(31);
    std::cout << "lis $5" << "\n.word new" << "\njalr $5" << std::endl;
    pop(31);
    std::cout << "bne $3, $0, 1 ; if call succeeded skip next instr" << "\nadd $3, $11, $0 ; set $3 to NULL if alloc fails" << std::endl;
}

void codeDelete(std::shared_ptr<Node> statement, SymbolTable &varTable) {
    std::string id = std::to_string(getUniqueId());
    auto exprNode = statement->children.at(3);
    code(exprNode, varTable);
    std::cout << "beq $3, $11, skipDelete" << id << " ; do not call delete on NULL" << std::endl;
    std::cout << "add $1, $3, $0 ; delete expects addr in $1" << std::endl;
    push(31);
    std::cout << "lis $5" << "\n.word delete" << std::endl;
    std::cout << "jalr $5" << std::endl;
    pop(31);
    std::cout << "skipDelete" << id << ":" << std::endl;
}
...
void populateWainVar(std::shared_ptr<Node> wain, SymbolTable &varTable) {
    ...
    for (auto child : wain->children) {
        ...
        if (from == DCL) {
            auto typeNode = child->children.front();
            std::string type = typeNode->getTo();
            // determining if the program is meant to be run with mips.array
            if (param == 1 && type != INT_STAR) { 
                std::cout << "add $2, $0, $0" << std::endl; // set $2 to 0 if not run with mips.array
            }
